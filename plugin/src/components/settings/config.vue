<template>
  <div>
    ...
    <n-space vertical justify="start" :wrap-item="false">
      ...
      <div class="auto_ticket_config">
        <n-space style="margin-top: 20px">
          <n-text>Auto tickets</n-text>
        </n-space>
        <n-text depth="3" style="display: block; margin-bottom: 12px">
          Creates a support ticket after the instance payment date plus the
          delay for that billing period. Placeholders: {CLIENT_NAME},
          {INSTANCE}, {PRODUCT}, {IPS}, {SERVICE_DETAILS}, {SERVICE}
        </n-text>
        ...
        <n-space vertical style="margin-top: 12px; max-width: 720px">
          ...
          />
          <n-text depth="3">
            Used when the instance period is not listed below. Current default:
            24 hours.
          </n-text>
          ...
          <n-button ghost type="success" @click="addDelayRule"
            >Add period</n-button
          >
          ...
          <n-text>Topic</n-text>
          <n-input
            v-model:value="autoTicket.topic"
...
const autoTicket = reactive(parseAutoTicket({}));
...
function setDefaultConfig() {
  ...
  Object.assign(autoTicket, parseAutoTicket(defaultsStore.bot?.values));
}
