        ...
        <n-tooltip placement="top-end">
          <template #trigger>
            ...
            <n-mention
              ...
              @keyup.prevent.ctrl.shift.enter.exact="sendMode = 'admin'; handle_send()"
...
const SendIcon = defineAsyncComponent(() => import('@vicons/ionicons5/SendOutline'))
...
watch(() => store.updating, (value) => {
  if (value) {
    const message = store.current_message

    const { content, attachments } = message ?? {}
    const template = content?.match(/<div class="chat__files">[\s\S]{1,}<\/div>$/g)?.at(0)
    ...
    urls.forEach((url) => {
      ...
      fileList.value.push({ thumbnailUrl, name, uuid, id: uuid ?? '', status: 'finished' })
    })
...
async function handle_send() {
  if (fileList.value.length > 0) {
    ...
    const template = (fileList.value.length > 0)
      ? `<div class="chat__files">
          ${fileList.value.map((file) => `<div class="files__preview">
...
async function handle_send_files() {
  for await (const file of fileList.value) {
    ...
    const baseUrl = (store.baseUrl.endsWith('/')) ? store.baseUrl.slice(0, -1) : store.baseUrl
